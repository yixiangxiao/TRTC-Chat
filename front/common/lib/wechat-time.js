...
export function msgTimeFormat(currentMessageList, sort, type) {
    ...
    currentFilterList.forEach((item, index) => {
        ...
        if (index === 0) {
        //第一条必显示时间
            showTime = _getTimeStringAutoShort2(date, true)
			item.payload={text: showTime}
			item.isShowTime = true
            newMessageList.push({
                payload: {
                    text: showTime
                },
                isShowTime: true,//超五分钟显示时间-标识
                time: item.create_time
            })
			
        } else if (index <= currentFilterList.length - 1) { // 如果下标没用超过数组长度
            ...
            if (type) {
                ...
                minutes = (current - prev) / 60 /1000
            }
            if (!sort) {
                minutes = (current - lastShowTime) / 60/1000
            } else {
                minutes = (lastShowTime - current) / 60/1000
            }
			// console.log(minutes)
            //超五分
            if (minutes > 5) {
    ...
    // console.log('最后得到的数组', currentFilterList)
