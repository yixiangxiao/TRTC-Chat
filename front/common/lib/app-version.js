...
const $iconUrl = "/static/images/rowtos.png";
const $closeUrl = "/static/images/close.png";
const $mainColor = "#FF5B78";
class Upgrade {
	constructor() {
		this.downloadUrl = `${$C.baseUrl}/download/chat.apk`
		this.appInfo = null
	...
	async getLatestVersion() {
		try {
			...
			const res = await searchAppInfo({
				appid: plus.runtime.appid,
				apptype: platform
			})
			this.appInfo = res || { downloadurl_sign: this.downloadUrl }
			console.log(res)
			if (res) {
				console.log(res)
				return res.version
	...
	updatePackage() {
		if (/\.wgt/i.test(this.appInfo.downloadurl_sign) || (this.platform == 'android' && /\.apk/i.test(this.appInfo
				.downloadurl_sign))) {
			// 如果是热更新  wgt 或 android平台下apk
			let datas = {
				versionInfo: this.appInfo.content,
				versionName: this.appInfo.version,
				updateType: this.appInfo.update_type, //forcibly强制立即升级  solicit非强制升级
				downloadurl_sign: this.appInfo.downloadurl_sign
			}
			updatePopup(datas)
			return
		}
		...
		if (this.platform == 'ios') {
			// 如果是ios,则跳转到appstore
			plus.runtime.openURL(this.appInfo.downloadurl_sign)
			return;
		}
	}
}
...
function newUpdateDownload(data) {
	if (data.shichang === 1 && /\.apk/i.test(data.downloadurl_sign)) {
		...
		dtask = plus.downloader.createDownload(data.downloadurl_sign, {
				filename: "_downloads/"
			},
