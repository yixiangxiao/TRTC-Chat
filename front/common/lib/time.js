export default{
	...
	isTimestampExpired(timestamp) { // 是否超过两分钟
	  let now = Date.now();
	  let comparedTime = new Date(timestamp).getTime();
	  let twoMinutesInMilliseconds = 2 * 60 * 1000; // 2分钟的毫秒数
	  return (now - comparedTime) > twoMinutesInMilliseconds;
	}
}